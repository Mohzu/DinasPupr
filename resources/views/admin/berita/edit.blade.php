...
@section('page-description', 'Edit berita: ' . Str::limit($berita->judul, 50))
...
<div class="max-w-4xl mx-auto">
    ...
    <div class="bg-white rounded-xl p-6 shadow-sm border border-gray-100 mb-6">
        ...
        <p class="text-gray-500 mt-1 text-sm">Edit berita: {{ Str::limit($berita->judul, 50) }}</p>
    </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 lg:p-8">
        <form action="{{ route('admin.berita.update', $berita) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div>
                ...
                <input type="text" name="slug" id="slug" value="{{ old('slug', $berita->slug) }}" readonly
                ...
                <p class="mt-1 text-xs text-gray-500 flex items-center gap-1">
                    ...
                    Slug akan diupdate otomatis jika judul diubah
                </p>
            </div>
...
@push('scripts')
<script>
    // Auto-generate slug from judul (only if changed)
    const originalJudul = '{{ $berita->judul }}';
    document.getElementById('judul').addEventListener('input', function(e) {
        if (e.target.value !== originalJudul) {
