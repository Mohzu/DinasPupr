...
<div class="max-w-3xl mx-auto space-y-6">
    <div class="bg-gradient-to-r from-amber-500 via-orange-500 to-amber-600 text-white rounded-3xl shadow-2xl p-6 sm:p-8 space-y-5">
        <div class="space-y-2">
            <p class="uppercase tracking-[0.25em] text-xs font-semibold text-white/80">Pengaduan</p>
            <h2 class="text-2xl sm:text-3xl font-bold leading-snug">{{ $pengaduan->subjek }}</h2>
        </div>
        <div class="flex flex-wrap gap-4 text-sm sm:text-base">
            <div class="flex items-center gap-3">
                <div class="w-10 h-10 rounded-2xl bg-white/20 flex items-center justify-center font-bold text-white text-lg">
                    {{ Str::upper(Str::substr($pengaduan->nama, 0, 1)) }}
                </div>
                <div>
                    <p class="font-semibold">{{ $pengaduan->nama }}</p>
                    <p class="text-white/80">{{ $pengaduan->email }}</p>
                </div>
            </div>
        ...
        <div class="flex flex-wrap gap-4 text-white/80 text-sm">
            <div class="flex items-center gap-2">
                ...
                Diterima: {{ $pengaduan->created_at->format('d M Y, H:i') }}
            </div>
            <div class="flex items-center gap-2">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3"/>
                </svg>
                Terakhir diperbarui: {{ $pengaduan->updated_at->format('d M Y, H:i') }}
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-3xl shadow-xl border border-gray-100 p-6 sm:p-8 space-y-8">
        <div class="grid sm:grid-cols-2 gap-4">
            <div class="p-4 rounded-2xl bg-gray-50 border border-gray-100">
                ...
                <p class="font-semibold text-gray-800 break-all">{{ $pengaduan->email }}</p>
            </div>
            <div class="p-4 rounded-2xl bg-gray-50 border border-gray-100">
                <p class="text-sm text-gray-500 mb-1">Status Saat Ini</p>
                <p class="font-semibold text-gray-800">{{ ucfirst($pengaduan->status) }}</p>
            </div>
        </div>
        ...
        <form action="{{ route('admin.pengaduan.update', $pengaduan) }}" method="POST" class="space-y-6">
            ...
            <div>
                ...
                <div class="relative">
                    ...
                    <select name="status" id="status" required
                            class="w-full pl-12 pr-4 py-3 rounded-2xl border border-gray-200 focus:ring-4 focus:ring-amber-100 focus:border-amber-500 bg-white text-gray-800 font-medium">
            ...
            <div class="flex flex-wrap items-center justify-end gap-4 pt-4 border-t border-dashed border-gray-200">
                ...
                <button type="submit" 
                        class="px-8 py-3 rounded-2xl bg-gradient-to-r from-amber-500 to-orange-500 text-white font-semibold shadow-lg hover:shadow-xl hover:scale-[1.01] transition-all">
                    Simpan Perubahan
                </button>
            </div>
        </form>
