...
@section('title', 'Edit Dokumen - Admin Panel')
@section('page-title', 'Edit Dokumen')
@section('page-description', 'Edit dokumen: ' . Str::limit($dokumen->title, 50))
...
<div class="max-w-4xl mx-auto">
    ...
    <div class="bg-white rounded-xl p-6 shadow-sm border border-gray-100 mb-6">
        ...
        <p class="text-gray-500 mt-1 text-sm">Edit dokumen: {{ Str::limit($dokumen->title, 50) }}</p>
    </div>

    <!-- Form -->
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 lg:p-8">
        <form action="{{ route('admin.dokumen.update', $dokumen) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <input type="text" name="title" id="title" value="{{ old('title', $dokumen->title) }}" required
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="4"
                          ...
                          placeholder="Masukkan deskripsi dokumen (opsional)">{{ old('description', $dokumen->description) }}</textarea>
                @error('description')
                    <p class="mt-1 text-sm text-red-600 flex items-center gap-1">
                        <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
                            <path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7 4a1 1 0 11-2 0 1 1 0 012 0zm-1-9a1 1 0 00-1 1v4a1 1 0 102 0V6a1 1 0 00-1-1z" clip-rule="evenodd"/>
            ...
            <div>
                ...
                <input type="number" name="year" id="year" value="{{ old('year', $dokumen->year) }}" required min="1900" max="3000"
                       class="w-full px-4 py-2.5 rounded-lg border border-gray-200 focus:ring-2 focus:ring-purple-500 focus:border-purple-500 transition-all outline-none"
            ...
            <!-- File -->
            <div>
                <label for="file_path" class="block text-sm font-semibold text-gray-700 mb-2">File Dokumen (PDF)</label>
                @if($dokumen->file_path)
                    <div class="mb-4 p-4 bg-gray-50 rounded-lg border border-gray-200">
                        <p class="text-sm font-medium text-gray-700 mb-2">File saat ini:</p>
                        <a href="{{ asset('storage/' . $dokumen->file_path) }}" target="_blank" 
                           class="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-gradient-to-r from-purple-50 to-violet-50 text-purple-700 hover:from-purple-100 hover:to-violet-100 border border-purple-200 transition-all">
                    ...
                    <div class="space-y-1 text-center">
                        ...
                        <div class="flex text-sm text-gray-600">
                            <label for="file_path" class="relative cursor-pointer bg-white rounded-md font-medium text-purple-600 hover:text-purple-500 focus-within:outline-none focus-within:ring-2 focus-within:ring-offset-2 focus-within:ring-purple-500">
                                ...
                                <input type="file" name="file_path" id="file_path" accept=".pdf" class="sr-only">
                            </label>
                            <p class="pl-1">atau drag and drop</p>
                        </div>
                        <p class="text-xs text-gray-500">Kosongkan jika tidak ingin mengubah. PDF hingga 10MB</p>
                    </div>
                </div>
                @error('file_path')
                    <p class="mt-1 text-sm text-red-600 flex items-center gap-1">
            ...
            <div>
                ...
                <input type="date" name="published_at" id="published_at" 
                       value="{{ old('published_at', $dokumen->published_at ? $dokumen->published_at->format('Y-m-d') : now()->format('Y-m-d')) }}"
                       class="w-full px-4 py-2.5 rounded-lg border border-gray-200 focus:ring-2 focus:ring-purple-500 focus:border-purple-500 transition-all outline-none">
                @error('published_at')
                    <p class="mt-1 text-sm text-red-600 flex items-center gap-1">
                        <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
